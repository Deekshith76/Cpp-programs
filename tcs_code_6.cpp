...
// finding the indices of source and destination points
for(int i=0; i<8; i++){
    if(s==arrs[i])
    s_index = i;
...
if(s_index == d_index){
    ...
    return 0; //come out of the program
}else{
    int i,j;
    if(s_index<7 ){
        i = s_index + 1; //points the next corresponding stop
    } else{
        i = (s_index + 1) - 8;
    }
    
    if(d_index<7 ){
        j = d_index + 1; //points the next corresponding stop
    } else{
        j = (d_index + 1) - 8;
    }
    ...
    while(i != j){
        res += arr[i];
        i = (i+1)%8; //moving to next position 
    }
    cout << (ceil)(res*0.005) << " INR"<<endl;
}
